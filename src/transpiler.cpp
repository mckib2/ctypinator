...
public:
  explicit TranspilationConsumer(ASTContext *Context, const char* outfile)
    : Visitor(Context), outfile(outfile) {}
...
public:
  TranspilationAction(const char *const outfile) : outfile(outfile) {}
  virtual std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(
        clang::CompilerInstance &Compiler, llvm::StringRef InFile) {
    Preprocessor &pp = Compiler.getPreprocessor();
    pp.addPPCallbacks(std::unique_ptr<FindDefines>());
    return std::unique_ptr<clang::ASTConsumer>(
        new TranspilationConsumer(&Compiler.getASTContext(), outfile));
  }
private:
  const char *const outfile;
};
...
extern "C" {
  void transpile(const char *const code, const char *const outfile) {
    clang::tooling::runToolOnCode(std::make_unique<TranspilationAction>(outfile), code);
  }
}
